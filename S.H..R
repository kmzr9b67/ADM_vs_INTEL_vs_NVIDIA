...
int1 <- csv_in[,c(1,5)]
#Ocena kompletności danychi INTELL 
#NVIDIA
...
#DLA AMD I NVIDIA: dane są kompletne -0 oznacza, zakończne sprawdzanie ale nic nie zostało wypisane ,
#więc wszystkie są nie NA
...
max(sl_i)
#INTEL 
#Przedstaw uzyskane szeregi czasowe w postaci wykresu/wykresów.
#NVIDIA 
...
#INTEL i NVIDIA 
cor(sl_i,sl_n)
#Wyznacz skośność i kurtozę dla stóp zwrotu z każdego z instrumentów. 
#NVIDIA 
N<- coredata(sl_n)
...
hist_2<-ggplot(data.frame(AMD), aes(x = AMD)) +
  ...
  geom_histogram(binwidth = bwdth, color = "white", fill = "darkgreen", linewidth = 0.1) +
  stat_function(fun = function(x) dnorm(x) * 1250 * bwdth, color = "red", linewidth = 1.5)
hist_3<-ggplot(data.frame(INTELL), aes(x = INTELL)) +
  theme_bw() +
  geom_histogram(binwidth = bwdth, color = "white", fill = "darkgreen", linewidth = 0.1) +
  stat_function(fun = function(x) dnorm(x) * 1250 * bwdth, color = "red", linewidth = 1.5)
